...
class utils(object):
	...
	def prediction(self,preds,labels,mode='at'):
		# get current file list
		lst, _ = self.get_vali_lst()
		preds = preds[:len(lst)]
		#print(preds)
		label_list = ['Alarm_bell_ringing','Blender','Cat','Dishes','Dog','Electric_shaver_toothbrush','Frying','Running_water','Speech','Vacuum_cleaner','gunfire','car','crying','fire','fireworks','glass','screaming','explosion']
		...
		for i in range(len(lst)):
			...
			for j in range(len(label_list)):
				#t.append(preds[i][j] * 100)
				t[label_list[j]] = preds[i][j] * 100
			print(t)
		...
		print(preds)
		#if mode=='sed':
			#self.get_sed_result(preds,labels)
		return 0
